...
class Weather(commands.Cog):
    ...
    async def _weather_geolocate(self, zipcode: int) -> tuple:
        """geolocate zipcode"""
        try:
    ...
    async def _weather_get_data(self, lat: float, lon: float) -> list:
        """get weather data"""
        try:
            json_data = await self.ass.get_url_data(
    ...
    async def _weather_get_embed(self, city: str, weather_data: list) -> Embed:
        """get weather embed"""
    ...
    async def _weather_mm_to_in(self, mm: float) -> float:
        """mm to in"""
        return mm * 0.0393701
